...
module cpu_core(
    ...
    reg        ex_mem_is_mtc0;
    reg        ex_mem_is_link;
    reg        ex_mem_is_syscall, ex_mem_is_break, ex_mem_is_eret;
    ...
    always @* begin
        ...
        case(id_op)
            ...
            // [标准修改] LUI 使用 OR 操作，立即数在 imm 生成时移位
            6'h0F: begin ctrl_alu_src = 1; ctrl_reg_write = 1; ctrl_alu_op = 4'h3; end // LUI -> OR
            6'h20, 6'h21, 6'h23, 6'h24, 6'h25: begin // Load
                ctrl_alu_src = 1; ctrl_mem_to_reg = 1; ctrl_reg_write = 1; ctrl_mem_read = 1; ctrl_alu_op = 4'h0;
            end 
            6'h28, 6'h29, 6'h2B: begin // Store
            ...
            6'h01: begin // BGEZ/BLTZ/AL
                ctrl_branch = 1;
                if(id_rt==16||id_rt==17) begin ctrl_is_link = 1; ctrl_reg_write = 1; ctrl_reg_dest = 0; end 
            end 
            6'h02: ctrl_jump = 1; // J
            ...
            6'h10: begin // COP0
                ...
                else if (if_id_instr[25:0] == 26'b10000000000000000000011000) ctrl_is_eret = 1; // ERET
            end
        endcase
    end
    ...
    // [标准修改] LUI 指令将立即数左移 16 位，其他指令根据 ctrl_imm_zero 选择扩展方式
    wire [31:0] final_imm    = (id_op == 6'h0F) ? {id_imm, 16'b0} : 
                               (ctrl_imm_zero ? zero_ext_imm : sign_ext_imm);
    ...
    assign flush = branch_taken || jump_taken || ex_mem_is_eret;
    ...
    always @* begin
        if (mem_wb_is_link) begin
            // [标准修正] 链接地址为 PC + 8
            // 这里的 mem_wb_pc 是传递下来的 if_id_pc (即当前指令 PC+4)
            // 所以 PC+8 = mem_wb_pc + 4
